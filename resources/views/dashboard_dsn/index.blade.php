            ...
            @foreach($prodi->students as $index => $student)
              <tr>
                ...
                <form action="{{ route('persetujuan.store') }}" method="POST">
                  @csrf
                  <td>
                    @foreach($prodi->detailCutiMhs as $detailCuti)
                      <div class="pretty p-icon p-round p-plain p-smooth">
                        <input type="radio" name="status_persetujuan_pa" value="disetujui" {{ ($detailCuti->status_persetujuan_pa === 'disetujui') ? 'checked disabled' : '' }}>
                      ...
                      <div class="pretty p-icon p-round p-plain p-smooth">
                          <input type="radio" name="status_persetujuan_pa" value="ditolak" {{ ($detailCuti->status_persetujuan_pa === 'ditolak') ? 'checked disabled' : '' }}>
                  ...
                  <td>
                    ...
                    data-bs-toggle="modal" data-bs-target="#modal_{{ $detailCuti->students->nama }}"
                    >
                        <i class="fa-solid fa-arrow-up-right-from-square"></i>
                        <span class="hidden lg:inline-block">Submit</span>
                    </button>

                      <div class="modal fade fixed top-0 left-0 right-0 hidden w-full h-full outline-none overflow-x-hidden overflow-y-auto z-10 backdrop-brightness-50 bg-white/30" id="modal_{{ $detailCuti->students->nama }}" tabindex="-1" aria-labelledby="exampleModalCenterTitle" aria-modal="true" role="dialog">
                        ...
                        <div class="modal-dialog modal-dialog-centered relative flex justify-center self-center w-auto pointer-events-none">
                          ...
                          <div class="modal-content border-none shadow-lg flex flex-col mt-40 w-4/5 lg:w-3/6 pointer-events-auto bg-white bg-clip-padding rounded-md outline-none text-current">
                            ...
                            class="modal-footer flex flex-shrink-0 flex-wrap items-center justify-end p-4 border-t border-gray-200 rounded-b-md">
                              ...
                              @csrf
                              @method('POST')
                              <?php var_dump($detailCuti->students->nim);?>
                              <button type="submit"
