...
@section('content')
  ...
  <div class="container grid p-2 gap-2 w-full">
    ...
    <div class="overflow-x-auto bg-slate-50 shadow-md p-3 rounded-md">
      {{-- <table class="min-w-full pt-2" width="100%" cellspacing="0" id="tabel-data" >
        <thead class="border-b">
          <tr class="bg-indigo-100 border-indigo-200 border-2">
            <th scope="col" class="text-sm font-medium text-gray-900 px-2 py-4 text-center border-indigo-200 border-2">
              No.
            </th>
            <th scope="col" class="text-sm font-medium text-gray-900 px-2 py-4 text-center border-indigo-200 border-2">
              Username
        ...
        <tbody>
          ...
          <tr class="border-b">
              <td class="px-2 py-4 whitespace-nowrap text-sm font-medium text-gray-900 text-center border-indigo-200 border-2">{{ $index + 1 }}</td>
              <td class="text-sm text-gray-900 font-light px-2 py-4 whitespace-nowrap border-indigo-200 border-2">{{$user->username}}</td>
              <td class="text-sm text-gray-900 font-light px-2 py-4 whitespace-nowrap border-indigo-200 border-2">{{$user->role}}</td>
              <td class="text-sm text-gray-900 font-light px-2 py-4 whitespace-nowrap border-indigo-200 border-2">{{$user->email}}</td>
              <td class="sm:px-2 border-indigo-200 border-2">
                <ul class="flex justify-center gap-2">
                    <a href="{{ route('user.edit', $user->id) }}" class="inline-block px-3 py-2.5 bg-yellow-500 text-white font-medium text-xs lg:leading-tight uppercase rounded shadow-md hover:bg-yellow-600 hover:shadow-lg focus:bg-yellow-600 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-yellow-700 active:shadow-lg transition duration-150 ease-in-out">
                    ...
                    <form action="{{ route('user.destroy', $user->id) }}" method="POST">
                        ...
                        <div class="modal fade fixed top-0 left-0 right-0 hidden w-full h-full outline-none overflow-x-hidden overflow-y-auto z-10 backdrop-brightness-50 bg-white/30" id="modal_{{ $user->id }}" tabindex="-1" aria-labelledby="exampleModalCenterTitle" aria-modal="true" role="dialog">
                          ...
                          <div class="modal-dialog modal-dialog-centered relative flex justify-center self-center w-auto pointer-events-none">
                            ...
                            <div class="modal-content border-none shadow-lg flex flex-col mt-40 lg:w-2/5 pointer-events-auto bg-white bg-clip-padding rounded-md outline-none text-current">
